...
const replace = async (studentId, userId, body, files) => {
  if (!mongoose.Types.ObjectId.isValid(studentId)) {
    throw new NotFoundError(`Invalid student ID format`);
  }

  const foundStudent = await Student.findById(studentId).select('owner');
  if (!foundStudent) throw new NotFoundError(`student with id ${studentId} not found`);
  if (foundStudent.owner.toString() !== userId) throw new ForbiddenError('Not your student');
...
const deleteOne = async (studentId, userId) => {
  ...
  if (!deletedStudent) throw new NotFoundError(`student with id ${studentId} not found`);
  if (deletedStudent.owner.toString() !== userId) throw new ForbiddenError('Not your student');
